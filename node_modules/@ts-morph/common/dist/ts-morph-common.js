...
class RealFileSystemHost {
    ...
    getDirectoryNotFoundErrorIfNecessary(err, path) {
        return FileUtils.isNotExistsError(err) ? new exports.errors.DirectoryNotFoundError(path) : err;
    }
    getFileNotFoundErrorIfNecessary(err, path) {
        return FileUtils.isNotExistsError(err) ? new exports.errors.FileNotFoundError(path) : err;
    }
}
...
class TransactionalFileSystem {
    ...
    readDirSync(dirPath) {
        ...
        for (const childDirOrFilePath of this.fileSystem.readDirSync(dirPath)) {
            if (!this.isPathQueuedForDeletion(childDirOrFilePath))
                uniqueDirPaths.add(childDirOrFilePath);
        }
...
class InMemoryFileSystemHost {
    ...
    deleteSync(path) {
        ...
        if (parentDir == null || !parentDir.files.has(path))
            throw new exports.errors.FileNotFoundError(path);
    ...
    readDirSync(dirPath) {
        ...
        if (dir == null)
            throw new exports.errors.DirectoryNotFoundError(dirPath);
    ...
    readFileSync(filePath, encoding = "utf-8") {
        ...
        if (parentDir == null)
            throw new exports.errors.FileNotFoundError(filePath);
    ...
    moveSync(srcPath, destPath) {
        ...
        if (this.fileExistsSync(srcPath)) {
            ...
            throw new exports.errors.PathNotFoundError(srcPath);
        }
    }
    copy(srcPath, destPath) {
...
function createHosts(options) {
    ...
    const compilerHost = {
        ...
        getDirectories: (path) => transactionalFileSystem.getDirectories(path),
...
function createModuleResolutionHost(options) {
    ...
    return {
        ...
        getCurrentDirectory: () => transactionalFileSystem.getCurrentDirectory(),
        getDirectories: path => {
            const dirs = new Set(transactionalFileSystem.readDirSync(path));
